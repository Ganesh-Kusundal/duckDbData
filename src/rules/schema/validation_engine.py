...
class RuleValidator:
    ...
    def __init__(self, schema_path: str = None):
        ...
        if schema_path is None:
            import os
            # Get the directory of this file and construct the schema path
            current_dir = os.path.dirname(os.path.abspath(__file__))
            schema_path = os.path.join(current_dir, 'rule_schema.json')
    ...
    def _validate_semantics(self, rule: Dict[str, Any], result: ValidationResult):
        ...
        if not rule_id.replace('-', '').replace('_', '').replace('.', '').isalnum():
            result.add_error('rule_id', 'Rule ID must contain only alphanumeric characters, hyphens, underscores, and dots')
