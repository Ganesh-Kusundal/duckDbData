...
class BreakoutRuleMapper:
    ...
    def _convert_signal_to_scanner_format(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Try to get price_change_pct from metadata > market_data_snapshot
        ...
        # Fallback to direct signal field if available
        if 'price_change_pct' in signal and price_change_pct == 1.0:
            price_change_pct = signal['price_change_pct']

        # Get additional price data from market_data_snapshot
        price_at_0950 = None
        price_at_1515 = None
        ...
        if 'metadata' in signal and signal['metadata']:
            ...
            # Debug logging
            if signal.get('symbol') == 'INTELLECT':
                logger.info(f"DEBUG INTELLECT: metadata keys: {list(signal.get('metadata', {}).keys())}")
        ...
        return {
            ...
            'current_high': price * 1.01,  # Estimate
            'current_low': price * 0.99,   # Estimate
            ...
            'probability_score': signal.get('confidence', 0) * 100,
