...
class CRPRuleMapper:
    ...
    def _convert_signal_to_scanner_format(self, signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'crp_price': price,
            'open_price': signal.get('open_price', price),
            'current_high': signal.get('current_high', price * 1.01),
            'current_low': signal.get('current_low', price * 0.99),
            'current_volume': signal.get('volume', 0),
            'current_range_pct': signal.get('current_range_pct', 2.0),
            'close_score': signal.get('close_score', 0.4),
            'range_score': signal.get('range_score', 0.3),
            'volume_score': signal.get('volume_score', 0.2),
            'momentum_score': signal.get('momentum_score', 0.1),
            'crp_probability_score': signal.get('confidence', 0) * 100,
            'close_position': signal.get('close_position', 'Near High'),
            'entry_price': signal.get('entry_price', price),
            'stop_loss': signal.get('stop_loss'),
            'take_profit': signal.get('take_profit'),
            'performance_rank': signal.get('confidence', 0) * 100,
